...
We have set up and configured our GRASS session, so now we need to import raster and vector data into the GRASS mapset, which is a collection of layers that GRASS operations can refer to. For rasters, for example, our landuse raster, this is done with
```{r}
raster_to_mapset(lus, as_integer = TRUE, overwrite = TRUE)
```

There is no need to use the `raster_to_mapset()` function for the DEM because it was passed as an argument to the `set_envir()` function, which automatically imports any raster that is passed to it. Also, take note of the argument `as_integer`. This is `FALSE` by default, but it should be set to `TRUE` whenever you are importing an integer-valued raster. If you do not, GRASS may complain that the raster is "not of type CELL" in  
